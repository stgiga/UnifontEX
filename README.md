...
Additionally, all the circled and boxed letters, the checkboxes, radio buttons, check marks, X marks, the number+dot/comma, etc. al. and the consistent font pixel size make using UnifontEX for *creating* school assignments, surveys, or polls very attractive. Oh and yes, I've physically printed UnifontEX documents and they look fine. 

## TL;DR: UnifontEX's use cases could fill a book (literally). This README is lagging my Firefox, so I'm not going to start listing more (not to mention I dropped some letters on this latest commit, so don't expect a SourceForge readme update today due to how annoying those are to fix). Go enjoy UnifontEX! 
